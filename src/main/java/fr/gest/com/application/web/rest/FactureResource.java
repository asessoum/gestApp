...
/**
 * REST controller for managing {@link fr.gest.com.application.domain.Facture}.
 */
@RestController
@RequestMapping("/api")
public class FactureResource {
     ...
     */
    @GetMapping("/factures")
    public List<FactureDTO> getAllFactures(@RequestParam(required = false) String filter) {
        if ("commande-is-null".equals(filter)) {
            log.debug("REST request to get all Factures where commande is null");
            return factureService.findAllWhereCommandeIsNull();
        }
